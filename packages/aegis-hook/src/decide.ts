...
import { approvalId, consumeApproval, requestApproval } from './approval.js';
...
export interface DecideOptions {
  ...
  approvalDir?: string;
  /** Optional human-facing reason prefix for degraded/failure modes. */
  reasonOverride?: string;
  /** Predictor action key for telemetry/state joins. */
...
export function decide(evaluation: Evaluation, options: DecideOptions = {}): Decision {
  const displayReason = options.reasonOverride ?? evaluation.reason;
  switch (evaluation.action) {
    case 'deny':
      return { exitCode: 2, stderr: `[Aegis DENY] ${displayReason}` };
    case 'ask': {
      if (options.call !== undefined && consumeApproval(options.call, evaluation, options.approvalDir)) {
        return {
          exitCode: 0,
          stderr: `[Aegis ASK approved once] ${displayReason}`,
          approval: { event: 'consumed', id: approvalId(options.call, evaluation) },
        };
      }
      ...
      const pending = requestApproval(
        ...
        evaluation,
        options.approvalDir,
        options.approvalActionKey,
      );
      return {
        exitCode: 2,
        stderr:
          `[Aegis ASK requires approval] ${displayReason}\n` +
          `Approval id: ${pending.id}\n` +
          `Approve once: aegis-hook approve ${pending.id}\n` +
          `Then retry the exact same tool call without changing arguments, content, or paths.`,
        approval: { event: 'requested', id: pending.id },
      };
    }
    case 'allow':
    default:
