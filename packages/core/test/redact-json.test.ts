...
describe('redactJson — basic redaction', () => {
  ...
  it('matches token formats independent of key name', () => {
    const r = redactJson(
      { note: 'My key is ghp_EXAMPLEFAKEEXAMPLEFAKEEXAMPLEFAKE000 rotating soon' },
...
describe('redactContract / redactJson — parity (>=10 inputs)', () => {
  ...
  const CASES: Array<{ name: string; payload: Record<string, unknown> }> = [
    ...
    { name: 'github PAT in text', payload: { outputs: { note: 'leaked ghp_EXAMPLEFAKEEXAMPLEFAKEEXAMPLEFAKE000' } } },
    { name: 'aws access key', payload: { outputs: { note: 'AKIAIOSFODNN7EXAMPLE leaked' } } },
    { name: 'JWT token', payload: { outputs: { auth: 'eyJGQUtFX0hFQURFUg.eyJGQUtFX1BBWUxPQUQ.RkFLRV9TSUdOQVRVUkU' } } },
