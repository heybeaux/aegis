    ...
    it('redacts secrets embedded in constraint descriptions', () => {
      const contract = createContract({
        ...
        constraints: [
          {
            description:
              'fetch failed: token=sk_live_' + 'EXAMPLEFAKETESTKEY000000',
          },
        ],
        budget: { tokensUsed: 0, callsMade: 0, wallClockMs: 0 },
      });
