...
describe('effect outcome receipt binding', () => {
  async function fixture() {
    ...
    const store = {
      ...
      async completeEffect(operationId:string,receipt:any){if(completionFailures-->0)throw new Error('down');const e=effects.get(operationId);if(!e||e.state!=='started'&&e.state!=='committed')return'not_started';if(e.receipt){return JSON.stringify(e.receipt)===JSON.stringify(receipt)?'already_committed':'conflict';}e.receipt=structuredClone(receipt);e.state='committed';return'committed';},
    };
